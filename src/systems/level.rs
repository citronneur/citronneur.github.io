...
use crate::components::{BlackHole, GameObject, LevelManager, LevelState, PhysicsClock, Position, SpacecraftTag, Velocity};
...
pub fn system_level(world: &mut World, assets: &AssetManager, sound: &Sound, dt: f32, bh_renderer: &mut Option<BlackHoleRenderer>) {
    ...
    match state {
        ...
        LevelState::Translating => system_level_translating(world, assets, sound, dt, bh_renderer),
    }
}
...
fn system_level_translating(world: &mut World, assets: &AssetManager, sound: &Sound, dt: f32, bh_renderer: &mut Option<BlackHoleRenderer>) {
    ...
    let origin = Position { x : 50.0 , y : 460.0};
    ...
    for (_, (pos, vel, obj)) in world.query_mut::<(&mut Position, &mut Velocity, &GameObject)>() {
        pos.x += direction.x * 300.0_f32.powf(norm / 2048.0 + 0.1);
        pos.y += direction.y * 300.0_f32.powf(norm / 2048.0 + 0.1);
        if *obj == GameObject::Airship && (pos.x < 100.0 || pos.y < 100.0) {
